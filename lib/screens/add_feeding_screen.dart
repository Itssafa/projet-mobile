...
class _AddFeedingScreenState extends State<AddFeedingScreen> {
  ...
  Future<void> _saveFeeding() async {
    if (_formKey.currentState!.validate()) {
      ...
      await context.read<AnimalProvider>().addFeeding(feeding);

      if (mounted) {
        Navigator.pop(context);
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Plan d\'alimentation enregistré avec succès')),
        );
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Ajouter un plan d\'alimentation'),
      ),
      body: Form(
        key: _formKey,
        child: ListView(
